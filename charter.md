...
- **Advisory stack:** advisor = an independent **red-team POV** via `ask-advisor` — a *different* model if you have one (error-decorrelation), else your loop model + a hard adversarial persona (the value is the stance, not benchmark parity; see the skill) · council via `convene-council` (weekly + mandatory pre-pivot)
